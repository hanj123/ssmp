...
public class LogAop {
    ...
    @Before("execution(* com.WebController.*.*(..))")
...
//        executionClass = jp.getSignature().getClass();
...
//            executionMethod = executionClass.getMethod(methodName, classArgs); // 获取有参数方法
    ...
    @After("execution(* com.WebController.*.*(..))")
...
//                            methodAnnotation.value()[0];

                    // 获取访问时长
                    Long executionTime = new Date().getTime() - startTime.getTime(); // 将sysLog对象属性封装
                    ...
                    String username = ((User) (context.getAuthentication()
                            .getPrincipal())).getUsername();
                    sysLog.setUsername(username);

//                    sysLog.setMethod("[类名]" + executionClass.getName() +
//                            "[方法名]" + executionMethod.getName());
